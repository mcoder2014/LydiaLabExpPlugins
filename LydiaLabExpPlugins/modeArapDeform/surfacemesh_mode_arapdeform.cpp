...
void surfacemesh_mode_arapdeform::decorate()
{
    ...
    switch(currentMode){
        case CONTROL_FACES:
        info += "Mode: Control points: Size: " + QString::number(controlFaces.size());
        break;
    case ANCHOR_FACES:
        info += "Mode: Anchor points: Size: " + QString::number(anchorFaces.size());
        break;
        case DEFORM:
        {
            info += "Mode: Deform";
    ...
    switch(anchorMode){
        case MANUAL_POINTS: info += ", manual anchor";
        break;
        case GEODESIC_DISTANCE: info += ", distance anchors = " + QString::number(anchorDistance);
        break;
    }
...
void surfacemesh_mode_arapdeform::compute_geodesic_anchors()
{
    ...
    for(Vertex vertex: mesh()->vertices()){
        if(geo_dist[vertex] > anchorDistance)
            geo_dist[vertex] = 1;
        else
            geo_dist[vertex] = 0;
    }
